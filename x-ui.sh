...
update() {
    ...
    cd ~
    wget -N --no-check-certificate -O x-ui-install.sh https://
...
install() {
    cd ~
    wget -N --no-check-certificate -O x-ui-install.sh https://raw.githubusercontent.com/yonggekkk/x-ui-yg/ex/install.sh
...
update_shell() {
    wget -O ~/x-ui.sh -N --no-check-certificate https://raw.githubusercontent.com/yonggekkk/x-ui-yg/ex/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
